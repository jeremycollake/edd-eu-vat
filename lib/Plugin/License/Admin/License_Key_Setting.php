...
namespace Barn2\VAT_Lib\Plugin\License\Admin;

use Barn2\VAT_Lib\Admin\Settings_API_Helper;
use Barn2\VAT_Lib\Plugin\License\License;
...
class License_Key_Setting implements Registerable, License_Setting {
		...
		if ( $this->is_license_action( self::ACTIVATE_KEY ) ) {
			...
			if ( isset( $license_setting['license'] ) ) {
				...
				$this->add_settings_message(
					__( 'License key successfully activated.', 'edd-eu-vat' ),
					__( 'There was an error activating your license key.', 'edd-eu-vat' ),
					$activated
				);
			}
			...
			$this->add_settings_message(
				__( 'License key successfully deactivated.', 'edd-eu-vat' ),
				__( 'There was an error deactivating your license key, please try again.', 'edd-eu-vat' ),
				$deactivated
			);
			...
			$this->add_settings_message(
				__( 'The license key looks good!', 'edd-eu-vat' ),
				__( 'There\'s a problem with your license key.', 'edd-eu-vat' ),
	...
	public function get_license_key_setting() {
		$setting = [
			'title' => __( 'License key', 'edd-eu-vat' ),
			'type'  => 'text',
	...
	private function get_license_description() {
		$buttons = [
			'check'      => $this->license_action_button( self::CHECK_KEY, __( 'Check', 'edd-eu-vat' ) ),
			'activate'   => $this->license_action_button( self::ACTIVATE_KEY, __( 'Activate', 'edd-eu-vat' ) ),
			'deactivate' => $this->license_action_button( self::DEACTIVATE_KEY, __( 'Deactivate', 'edd-eu-vat' ) )
		];
