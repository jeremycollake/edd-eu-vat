...
namespace Barn2\Plugin\EDD_VAT\Dependencies\Lib\Plugin\License;
...
class Plugin_License implements Registerable, License
{
    ...
    public function get_status_help_text()
    {
        ...
        switch ($this->get_status()) {
            case 'active':
                $message = __('Your license key is active.', 'edd-eu-vat');
                break;
            case 'inactive':
                if (!$message) {
            ...
            default:
                if (!$this->get_license_key()) {
                    $message = __('Please enter your license key.', 'edd-eu-vat');
                } elseif (!$message) {
                    $message = __('Your license key is invalid.', 'edd-eu-vat');
                }
                break;
        }
        return $message;
    ...
    public function get_error_message()
    {
        ...
        switch ($this->get_error_code()) {
            case 'missing':
                ...
                break;
            case 'missing_url':
                $message = __('No URL was supplied for activation, please contact support.', 'edd-eu-vat');
                break;
            case 'key_mismatch':
                $message = __('License key mismatch, please contact support.', 'edd-eu-vat');
                break;
            case 'license_not_activable':
                $message = __('This license is for a bundled product and cannot be activated.', 'edd-eu-vat');
                break;
            case 'item_name_mismatch':
            case 'invalid_item_id':
                $message = __('Your license key is not valid for this plugin.', 'edd-eu-vat');
                break;
            case 'no_activations_left':
                $limit = '';
                if (isset($license_info['max_sites'])) {
                    /* translators: %s: The number of sites the license is activated on */
                    $limit = \sprintf(_n('%s site active', '%s sites active', \absint($license_info['max_sites']), 'edd-eu-vat'), $license_info['max_sites']);
                }
            ...
            case 'expired':
                ...
                if (!empty($license_info['expires']) && \is_numeric(\substr($license_info['expires'], 0, 4))) {
                    if ($expiry_datetime = DateTime::createFromFormat('Y-m-d H:i:s', $license_info['expires'])) {
                        ...
                        $message = \sprintf(__('Your license key expired on %s.', 'edd-eu-vat'), $expiry_datetime->format(\get_option('date_format')));
                    }
                }
                $renewal_link = Util::format_link($this->get_renewal_url(), __('Renew now for 20% discount.', 'edd-eu-vat'), \true);
                /* translators: support for RTL, 1: the license error, 2: a link */
    ...
    {
        return __('Your license key is not active. Please reactivate or save the settings.', 'edd-eu-vat');
    }
    private function get_license_expired_message()
    {
        return __('Your license key has expired.', 'edd-eu-vat');
    }
