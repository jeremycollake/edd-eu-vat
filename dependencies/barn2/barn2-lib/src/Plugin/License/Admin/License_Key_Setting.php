...
namespace Barn2\Plugin\EDD_VAT\Dependencies\Lib\Plugin\License\Admin;

use Barn2\Plugin\EDD_VAT\Dependencies\Lib\Admin\Settings_API_Helper;
...
class License_Key_Setting implements Registerable, License_Setting
{
    ...
    public function process_license_action()
    {
        if ($this->is_license_action(self::ACTIVATE_KEY)) {
            ...
            if (isset($license_setting['license'])) {
                ...
                $this->add_settings_message(__('License key successfully activated.', 'edd-eu-vat'), __('There was an error activating your license key.', 'edd-eu-vat'), $activated);
            }
        } elseif ($this->is_license_action(self::DEACTIVATE_KEY)) {
            $deactivated = $this->license->deactivate();
            $this->add_settings_message(__('License key successfully deactivated.', 'edd-eu-vat'), __('There was an error deactivating your license key, please try again.', 'edd-eu-vat'), $deactivated);
        } elseif ($this->is_license_action(self::CHECK_KEY)) {
            $this->license->refresh();
            $this->add_settings_message(__('The license key looks good!', 'edd-eu-vat'), __('There\'s a problem with your license key.', 'edd-eu-vat'), $this->license->is_active());
        }
    }
    ...
    {
        $setting = ['title' => __('License key', 'edd-eu-vat'), 'type' => 'text', 'id' => $this->get_license_setting_name() . '[license]', 'desc' => $this->get_license_description(), 'class' => 'regular-text'];
        if ($this->is_woocommerce) {
            $setting['desc_tip'] = __('The licence key is contained in your order confirmation email.', 'edd-eu-vat');
        } elseif ($this->is_edd) {
            // EDD uses title case for setting names, so let's keep things consistent.
            $setting['title'] = __('License Key', 'edd-eu-vat');
    ...
    {
        $buttons = ['check' => $this->license_action_button(self::CHECK_KEY, __('Check', 'edd-eu-vat')), 'activate' => $this->license_action_button(self::ACTIVATE_KEY, __('Activate', 'edd-eu-vat')), 'deactivate' => $this->license_action_button(self::DEACTIVATE_KEY, __('Deactivate', 'edd-eu-vat'))];
        ...
        if ($this->license->is_active()) {
            ...
            if ($this->license->is_inactive() && $this->is_license_action('deactivate_key')) {
                ...
                $message = __('License key deactivated.', 'edd-eu-vat');
            } else {
